...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivinen_lataus_ei_muuta_saldoa(self):
        saldo_ennen_latausta = self.kortti.saldo_euroina()
        self.kortti.lataa_rahaa(-100)

        self.assertEqual(self.kortti.saldo_euroina(), saldo_ennen_latausta)

    def test_syo_edullisesti_kun_saldona_lounaan_hinta(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(kortti.saldo_euroina(), 0)

    def test_syo_maukkaasti_kun_saldona_lounaan_hinta(self):
